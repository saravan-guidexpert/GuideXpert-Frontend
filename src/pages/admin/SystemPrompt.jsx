...
const historyCardVariants = {
  ...
  show: (i) => ({
    opacity: 1,
    y: 0,
    transition: { duration: 0.32, delay: Math.min(i, 8) * 0.035, ease: historyCardEase },
  }),
};
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      <section className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
        <div className="px-4 sm:px-5 pt-3.5 pb-3 space-y-2.5 border-b border-gray-100">
          <div className="flex flex-wrap items-center justify-between gap-2">
            <div className="min-w-0">
              <h2 className="text-[15px] font-semibold text-primary-navy tracking-tight leading-tight">
                System prompt history
              </h2>
              <p className="mt-0.5 text-xs text-gray-500">
                Live version shows a green status · filter, copy, or restore
              </p>
            </div>
            <button
              type="button"
              ...
              className="inline-flex items-center gap-1.5 px-2.5 py-1.5 text-xs font-medium rounded-md border border-gray-200 text-gray-700 bg-white hover:bg-gray-50 disabled:opacity-50 transition-colors duration-200"
            >
          ...
          {history.length > 0 || historyLoading ? (
            <div className="flex flex-col lg:flex-row lg:items-center gap-2">
              <div
                className="inline-flex flex-wrap rounded-md border border-gray-200 bg-slate-50 p-0.5"
                role="group"
                ...
                {HISTORY_RANGE_OPTIONS.map((opt) => {
                  ...
                  return (
                    <button
                      ...
                      className={`px-2 sm:px-2.5 py-1 text-[11px] font-medium rounded transition-colors duration-200 ${
                        active
              ...
              <div className="flex flex-1 flex-wrap items-center gap-2 min-w-0">
                <label className="relative flex-1 min-w-[12rem]">
                  ...
                  <input
                    ...
                    className="w-full rounded-md border border-gray-200 bg-white px-2.5 py-1.5 text-xs text-gray-900 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-primary-navy/25 focus:border-primary-navy/40"
                  />
                </label>
                {filtersActive ? (
                  <button
                    ...
                    className="px-2 py-1.5 text-[11px] font-medium rounded-md border border-gray-200 bg-white text-gray-600 hover:bg-gray-50 transition-colors duration-200"
                  >
                    Reset
                  </button>
                ) : null}
          ...
          {historyRange === 'custom' ? (
            <div className="flex flex-wrap items-end gap-2.5">
              <label className="flex flex-col gap-0.5 text-[10px] font-medium uppercase tracking-wide text-gray-500">
                From
                <input
                  type="date"
                  value={historyFrom}
                  onChange={(e) => setHistoryFrom(e.target.value)}
                  className="rounded-md border border-gray-200 bg-white px-2 py-1 text-xs text-gray-900 focus:outline-none focus:ring-2 focus:ring-primary-navy/25"
                />
              </label>
              <label className="flex flex-col gap-0.5 text-[10px] font-medium uppercase tracking-wide text-gray-500">
                To
        ...
        {historyError ? (
          <div className="rounded-lg px-3.5 py-2.5 text-sm bg-red-50 text-red-800 border border-red-200">
            {historyError}
          </div>
        ) : null}
        ...
        {historyLoading && history.length === 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2.5">
            {[0, 1, 2].map((i) => (
              <div
                key={i}
                className="rounded-lg border border-gray-200 bg-white p-3 animate-pulse space-y-2 min-h-[9.5rem]"
              >
                ...
                <div className="h-[3.5rem] rounded-md bg-gray-100" />
              </div>
            ))}
          </div>
        ) : null}
        ...
        {!historyLoading && history.length === 0 && !historyError ? (
          <div className="rounded-lg border border-dashed border-gray-300 bg-slate-50/60 px-5 py-7 text-center">
        ...
        {!historyLoading && history.length > 0 && filteredHistory.length === 0 ? (
          <div className="rounded-lg border border-dashed border-gray-300 bg-slate-50/60 px-5 py-7 text-center">
            ...
            <p className="mt-1 text-xs text-gray-500">Widen the date range or clear the search query.</p>
            <button
              type="button"
              onClick={clearHistoryFilters}
              className="mt-3 px-3 py-1.5 text-xs font-medium rounded-md border border-gray-200 bg-white text-gray-700 hover:bg-gray-50 transition-colors duration-200"
            >
              Reset filters
            </button>
          </div>
            ...
            {filteredHistory.map((item, index) => {
              ...
              return (
                <motion.button
                  ...
                  whileHover={reduceMotion ? undefined : { y: -2, transition: { duration: 0.2, ease: historyCardEase } }}
                  whileTap={reduceMotion ? undefined : { scale: 0.985, transition: { duration: 0.12 } }}
                  className={`group relative flex flex-col text-left rounded-lg border bg-white p-3.5 focus:outline-none focus-visible:ring-2 focus-visible:ring-primary-navy/45 focus-visible:ring-offset-2 ${
                    isLive
                      ? 'border-emerald-200 shadow-[0_0_0_1px_rgba(16,185,129,0.12)] hover:border-emerald-300 hover:bg-emerald-50/20'
                      : 'border-gray-200 hover:border-primary-navy/30 hover:bg-slate-50/50'
                  }`}
                >
                  {isLive ? (
                    <span
                      className="absolute top-2.5 right-2.5 z-10 flex h-2.5 w-2.5"
                  ...
                  <div className={`min-w-0 ${isLive ? 'pr-5' : ''}`}>
                  ...
                  <div className="mt-2.5 flex-1 rounded-md border border-gray-100 bg-slate-50 px-2.5 py-2 min-h-[3.75rem]">
                  ...
                  <div className="mt-2.5 pt-2 border-t border-gray-100 flex items-center justify-between">
                    <span className="text-[11px] font-medium text-gray-500 group-hover:text-primary-navy transition-colors duration-200">
                      Open version
                    </span>
                    <span
                      aria-hidden="true"
                      className="text-gray-300 group-hover:text-primary-navy transition-all duration-200 text-sm leading-none motion-safe:group-hover:translate-x-0.5 inline-block"
                    >
                      →
                    </span>
                  </div>
      ...
      <AnimatePresence>
        {selectedId ? (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            transition={{ duration: 0.2, ease: historyCardEase }}
          >
            <motion.div
              ...
              transition={{ duration: 0.28, ease: historyCardEase }}
            >
              <div className="px-5 sm:px-6 py-3.5 border-b border-gray-100 flex flex-wrap items-start justify-between gap-3">
                <div className="min-w-0">
                  <div className="flex items-center gap-2">
                    <p className="text-[11px] font-semibold uppercase tracking-[0.14em] text-gray-500">
                      Saved version
                    </p>
                    {selectedItem && meta.hash && selectedItem.hash === meta.hash ? (
                ...
                <button
                  type="button"
                  onClick={closeHistoryModal}
                  className="shrink-0 inline-flex h-8 w-8 items-center justify-center rounded-lg border border-gray-200 text-gray-500 hover:bg-gray-50 hover:text-gray-800 transition-colors duration-200"
                  aria-label="Close"
                >
              ...
              <div className="px-5 sm:px-6 py-3.5 border-t border-gray-100 flex flex-wrap items-center justify-between gap-3 bg-slate-50/90 rounded-b-xl">
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="px-3.5 py-2 text-sm font-medium rounded-lg border border-gray-200 bg-white text-gray-700 shadow-sm hover:bg-gray-50 disabled:opacity-50 transition-colors duration-200"
                  >
                    Copy all
                  </button>
                  <button
                    ...
                    className="px-4 py-2 text-sm font-semibold rounded-lg bg-primary-navy text-white hover:bg-primary-navy/90 disabled:opacity-50 disabled:cursor-not-allowed transition-colors duration-200"
                  >
                    Load into editor
                  </button>
                </div>
