...
function AssistantReply({ content }) {
  ...
  return (
    <div
      className="gx-chat-reply max-w-[92%] rounded-2xl rounded-bl-md border border-[#e4e9f0] bg-white px-4 py-3.5 shadow-[0_1px_0_rgba(4,30,48,0.03)]"
...
export default function CollegeComparisonChat({ comparison }) {
  ...
  return (
    <div className="mt-6 overflow-hidden rounded-2xl border border-[#e4e9f0] bg-white">
      <div className="border-b border-[#eef2f7] bg-[#fbfcfe] px-4 py-4 sm:px-5">
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0">
            ...
            <p className="mt-1 text-sm leading-relaxed text-[#5a6570]">
              Get clear guidance on {aName} vs {bName} based on this comparison.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="p-4 sm:p-5">
        <div
          ref={listRef}
          className="max-h-80 space-y-3.5 overflow-y-auto rounded-xl bg-[#f8fafc] p-3.5 sm:p-4"
          aria-live="polite"
        >
          ...
          {loading ? <ChatTypingIndicator label="GuideXpert is preparing an answer" /> : null}
        </div>
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            {QUICK_PROMPTS.map((prompt) => (
              <button
                ...
                disabled={loading}
                className="rounded-full border border-[#dce3ec] bg-white px-3 py-1.5 text-left text-xs font-medium text-[#334155] transition hover:border-[#f27921] hover:text-[#c45a0c] disabled:opacity-60"
              >
                {prompt}
              </button>
            ))}
